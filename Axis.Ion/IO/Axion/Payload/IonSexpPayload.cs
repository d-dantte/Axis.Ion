...
{
    public readonly struct IonSexpPayload : ITypePayload
    {
        ...
        public static IonSexpPayload Read(
            Stream stream,
            TypeMetadata metadata,
            SerializerOptions options,
            SymbolHashList symbolTable)
        {
            if (metadata.IonType != IonTypes.Sexp)
            ...
            else
            {
                ...
                return itemCount
                    ...
                    .ApplyTo(items => new IonSexp.Initializer(
                        annotations,
                        items.ToArray()))
                    .ApplyTo(initializer => new IonSexpPayload(new IonSexp(initializer)));
            }
        }
        #endregion
