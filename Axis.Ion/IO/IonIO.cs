    ...
    public static class IonIO
    {
        ...
        internal static string? ToString(CSTNode ionString)
        {
            ...
            return stringKind.SymbolName switch
            {
                ...
                "regular-string" => stringKind
                    .FindNode("d-unquoted-text")
                    .TokenValue()
                    .ApplyTo(ApplyEscapeForQuotedString),

                "multiline-string" => stringKind
                    .FindNodes("block-space|ml-string")
                    .Select(node =>
                    {
                        return node.SymbolName switch
                        {
                            "ml-string" => node.FindNode("s-unquoted-text").TokenValue(),

                            "block-space" => NewLines(ExtractIndependentNewLineCount(node)),
                            _ => throw new InvalidOperationException($"Invalid symbol node: {node.SymbolName}")
                        };
                    })
                    .JoinUsing("")
        ...
        internal static byte[]? ToClobBytes(CSTNode ionClob)
        {
            ...
            {
                "null-clob" => null,
                "clob-value" => valueNode.FindNode("regular-string|clob-ml-string") switch
                {
                    CSTNode rstring when rstring.SymbolName.Equals("regular-string") => rstring
                        .FindNode("d-unquoted-text")
                        .TokenValue()
                    ...
                    CSTNode cmstring => cmstring
                        ...
                        .Select(child =>
                        {
                            return child.SymbolName switch
                            {
                                "ml-string" => child.FindNode("s-unquoted-text").TokenValue(),
                                "char-space" => child.TokenValue() switch
                                {
                                    "\r" => "\r",
                                    "\n" => "\n",
                                    _ => ""
                                },
                                _ => throw new InvalidOperationException($"Invalid symbol: {child.SymbolName}")
                            };
                        })
                        .JoinUsing("")
                        .ApplyTo(Encoding.Unicode.GetBytes),
