using Axis.Ion.IO;
using Axis.Ion.IO.Text.Serializers;
using Axis.Ion.Types;
using Axis.Luna.Common.Results;
using Axis.Luna.Extensions;

namespace Axis.Ion.Tests.IO.Text.Serializers
{
    [TestClass]
    public class IonAnnotationStreamerTest
    {
        [TestMethod]
        public void StreamText_ReturnsStringRepresentation()
        {
            var annotation = IIonValue.Annotation.Parse("plain_annotation").Resolve();
            var annotationText = IonAnnotationSerializer.Serialize(new[] { annotation });

            Assert.AreEqual($"{annotation.Value}::", annotationText);
        }

        [TestMethod]
        public void StreamText_WithAnnotationArray_ReturnsStringRepresentation()
        {
            var annotation = IIonValue.Annotation.Parse("plain_annotation").Resolve();
            var annotation2 = IIonValue.Annotation.Parse("'another'").Resolve();
            var annotationsText = IonAnnotationSerializer.Serialize(new[] { annotation, annotation2 });

            Assert.AreEqual($"{annotation.Value}::{annotation2.Value}::", annotationsText);
        }

        [TestMethod]
        public void Parse_WithValidTokenString_ReturnsAnnotationArray()
        {
            var annotationText = "$the_identifier::";
            var annotation = IonTextSerializer
                .Parse<IIonValue.Annotation[]>(annotationText)
                .Resolve();

            Assert.IsNotNull(annotation);
            Assert.AreEqual(1, annotation.Length);
            Assert.IsTrue(annotationText.StartsWith(annotation[0].Value));

            annotationText = "$the_identifier::'and others'::";
            annotation = IonTextSerializer
                .Parse<IIonValue.Annotation[]>(annotationText)
                .Resolve();

            Assert.IsNotNull(annotation);
            Assert.AreEqual(2, annotation.Length);
            Assert.AreEqual(
                annotationText,
                annotation.Select(x => $"{x.Value}::").JoinUsing(""));
        }
    }
}
